...
sacct --start=${startDate} --end=${endDate} --format=job,user,jobname,state,partition,TimeLimit,start,end,AllocNodes,NNodes,Submit --state=CD,TO,F,NF -p --delimiter=' ' --partition=regular,debug --noheader -X --duplicates -a > $rawLog
...
awk '{print "JobId="$1 " UserId="$2 " Name="$3 " JobState="$4 " Partition="$5 " TimeLimit="$6 " StartTime="$7 " EndTime="$8 " NNode="$9 " NodeCnt="$10 " SubmitTime="$11}' tmp.log > ${outfile}.log
