...
# parse input format and create a hash for each job
  ...
  while (<>) {
    ...
    my ($submit, $stime, $etime);
    my $j;
  #no data on submit time
    #add this in.
# get submit, wait, and run times
    # translate human readable date and time into time array
    my @subtmarr;
    if ($line[10] =~ /^SubmitTime=(\d+)(\W)(\d+)-(\d+):(\d+):(\d+)(.*)$/) {
      $subtmarr[5] = 2007 - 1900;
      $subtmarr[4] = $1 - 1;
    }
    elsif ($line[10] =~ /^SubmitTime=(\d+)-(\d+)-(\d+)T(\d+):(\d+):(\d+)(.*)$/) {
      $subtmarr[5] = $1 - 1900;
      $subtmarr[4] = $2 - 1;
    }
    else {
      warn ">>> can't parst start time $line[10]";
    }
    $subtmarr[3] = $3;
    $subtmarr[2] = $4;
    $subtmarr[1] = $5;
    $subtmarr[0] = $6;
    $submit = &timelocal(@subtmarr);
    if ($line[10] !~ /12\/31-16:00:00/) {    # logging bug!
      $nj{submit}  = $submit;
    }
